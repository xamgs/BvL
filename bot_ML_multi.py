...
def formatar_mensagem(produto):
    emojis = ["🔥", "💥", "⚡", "💰", "🛒"]
    frete = "Grátis" if produto.get('shipping', {}).get('free_shipping') else "Consultar"
    return (
        f"{random.choice(emojis)} *OFERTA IMPERDÍVEL!*\n\n"
        f"🛒 *{produto['title']}*\n"
        f"💰 *R$ {produto['price']:.2f}*\n"
        f"📦 Frete: {frete}\n\n"
        f"🔗 [COMPRAR AGORA]({gerar_link_afiliado(produto['permalink'])})\n\n"
        ...
        for categoria in CATEGORIAS:
            ...
            for p in produtos:
                if categoria["min"] <= p['price'] <= categoria["max"]:
                    ...
                    enviados += 1
                    time.sleep(3)  # Pequena pausa para não sobrecarregar
                    if enviados >= categoria["qtd"]:
                        break
            logging.info(f"{enviados} produtos enviados para a categoria {categoria['termo']}")
        logging.info(f"Aguardando {TEMPO_ENTRE_CICLOS} segundos para o próximo ciclo...")
